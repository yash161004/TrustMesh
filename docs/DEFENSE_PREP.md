...
I mitigated overfitting by strictly separating our test scenarios into training and holdout sets, specifically reserving an 8-scenario holdout group (Categories 1, 2, and 3) for the `ManipulationDetector`. It was testing against this unseen holdout data that exposed the extreme variance in our single-call LLM judgments, proving the benchmark was legitimately stressing the system and revealing real-world limitations rather than just confirming our biases.

**"n=13-17 scenarios is a small sample — how confident are you in these numbers?"**
I am confident that these scenarios map correctly to the specific structural failure modes we designed them to catch, like the "Volume Discount Trick" or the "Phantom Concession." However, I fully acknowledge that n=13 to 17 is absolutely not statistically powered to make broad claims about general industry readiness. It is a proof-of-concept taxonomy meant to demonstrate the engine's mechanics, and expanding this dataset is a primary goal for future work.
...
Evaluating subtle psychological pressure is inherently subjective, and I found that current LLMs exhibit severe binomial variance on these borderline cases depending on temperature and sampling. Across three documented validation runs on our holdout set, the exact same model swung from overly aggressive (2 False Positives) to perfect, to overly cautious (4 False Negatives). This non-determinism is a real characteristic of the underlying foundation models when dealing with ambiguous human-like negotiation tactics.

**"You proposed majority voting — does it work?"**
Mathematically, a 3-of-3 majority vote smooths out the binomial variance perfectly in our isolated tests. Practically, however, it fails on our current infrastructure because running three LLM evaluations per message instantly exhausts the 7,000 Tokens Per Minute limit of our free-tier provider. The mitigation works in principle, but we cannot validate it as a production-ready stream filter until we migrate to a paid tier with higher rate limits.
...
I would have skipped the attempt to use deterministic regex for claim extraction entirely and started immediately with an LLM structured-output approach. I also would have built the system around a paid-tier LLM from day one; spending days debugging what turned out to be hard free-tier rate limits significantly slowed down my validation of the majority-voting mitigation.
...
No, it is not. While the architectural foundation—the agents, the protocol layer, and the cryptographic ledger—is solid, the ManipulationDetector remains too unstable for production use without paid-tier majority voting. Furthermore, the benchmark sample size is too small to guarantee reliability against edge cases, and the system still lacks the per-session cryptographic identity required for real-world financial stakes.
