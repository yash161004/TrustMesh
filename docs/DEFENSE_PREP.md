...
Message signing uses a per-agent Ed25519 key (keyed on `agent_id`), with org-bound cards and a tenancy check enforced at sign time — so a compromised key's blast radius is that one agent's sessions, not every session of its role. The current caveat: because the API defaults `agent_id` to shared constants (`buyer-agent-001`/`seller-agent-001`), agents left on the default share one key file, which widens the blast radius until per-`(org, role)` identities land (corrected Phase 1 item #1). Key rotation/expiry is still not implemented — a compromised key must be rotated manually by regenerating the agent's keypair and re-issuing its card.
...
I have not yet implemented Phase D, the actual commerce and payments settlement layer, because my core research question was solving the trust and verification problem during the negotiation itself. The AgentCard identity layer (ERC-8004-style, per-agent Ed25519 keys, org-bound) is wired into the live signing path as of commit `6828ca7`; what remains is hardening it to be genuinely per-tenant — per-`(org, role)` identities in a DB-backed registry, the corrected Phase 1 item #1.

**"Is this production-ready?"**
Not yet. The PolicyDeviationFlagger and CommitmentConsistencyChecker both achieve 1.00 F1 and are structurally solid. The ManipulationDetector metrics show a regression under the current architecture (*Note: The recall regression identified in the 10-example few-shot prompt (0.67 recall, missing both Urgency-tactic scenarios — 'The Q4 Rush' and 'The Exploding Offer') has been resolved. Root cause: the contamination fix that swapped two few-shot examples for non-overlapping equivalents inadvertently removed the prompt's only Urgency-category anchor. Fix: swapped 'Protective Advisor' (a redundant Helpfulness-Disguise variant) for 'The Gradual Squeeze' (adversarial-6), using its final-turn message as a single-turn Urgency anchor. Note: adversarial-6 is tagged as multi-turn escalation (category 11) in the source data, not native Urgency (category 2) — it is used here because its final turn structurally matches the Urgency pattern, not because it is a native example of that category.\n\nConfirmed via 3 consecutive runs on the 8-scenario holdout: 1.00 precision / 1.00 recall / 1.00 F1 / 0.00 disagreement in all 3, with both previously-missed scenarios now caught and no regressions on any other scenario. (The original regression was observed across 3 pre-fix runs: two full runs at 0.67 recall, and one partial run cut short by API rate limits at 0.33 recall. Across all three, the only scenarios ever missed were the same two Urgency cases).*). An earlier report of poor calibration was due to a Brier/ECE scoring methodology bug (confident negative predictions were incorrectly scored as if they were confident positive predictions), which was found and fixed. The opt-in cross-provider voting path cannot be validated under real concurrency because free-tier rate limits prevent live 3-provider runs — which is precisely why self-consistency sampling on a single model (3 calls, temp 0.15, majority vote) is the shipped default, not a multi-provider ensemble. The total benchmark sample size (n=57 unique scenarios across all suites) is too small for general industry-readiness claims, and the system lacks per-session cryptographic identity for real-world financial stakes.
