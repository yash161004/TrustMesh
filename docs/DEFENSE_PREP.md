...
I mitigated overfitting by strictly separating our test scenarios into training and holdout sets. The strongest evidence against overfitting is our two-round Adversarial testing phase. After hitting 0.88 recall on the Tier 1 baseline (n=27 scenarios), we built a separate Adversarial Round 1 suite (n=18) targeting structural blind spots, where the detector maintained a strong 0.92 F1. When Round 1 revealed a weakness in detecting trust-exploitation, we developed a prompt fix independently, then validated it against a completely fresh Adversarial Round 2 holdout set (n=12). On that held-out set, recall improved from 0.20 to 0.50 with zero new false positives on the benign pairs. This proves the system learned the generalized pattern of manipulation rather than just memorizing the test scenarios.
...
Evaluating subtle psychological pressure is inherently subjective, and earlier single-call LLM runs exhibited severe binomial variance — swinging from overly aggressive (2 False Positives) to overly cautious (4 False Negatives) across identical holdout runs. The final stabilization was achieved after targeted fixes: (1) expanding the calibration anchor iteratively—first to a 3-example set which raised Tier 1 recall from 0.25 to 0.88, and later adding a 4th example (trust-exploitation) which raised recall on the Round 2 subset from 0.20 to 0.50 without triggering new false positives; (2) fixing a silent bug where provider exceptions were counted as "not manipulative" votes rather than being excluded; and (3) migrating to LiteLLM for standardized provider routing and retry handling.

**"You proposed majority voting — does it work?"**
The architecture correctly implements two-provider parallel voting (Groq + Gemini) with an OpenRouter tiebreaker, and we fixed a critical bug where failed provider calls were silently counted as False votes instead of being excluded. In the final benchmark, however, free-tier rate limits reduced the ensemble to effectively single-provider (Gemini): Groq contributed 1 live call out of 35, OpenRouter contributed 0. The 0.93 F1 reflects Gemini alone, not true multi-voter consensus. The code is verified to handle the multi-voter path correctly — a genuine 3-provider concurrent run is blocked by quota, not by bugs.
