...
- [x] **Harden AgentCard identity to be genuinely per-tenant. — Shipped 2026-07-23 (commit `9fd53cd`).** Provisioned one org-scoped `AgentIdentityRecord` per `(org_id, role)` with its own key, made the DB row the verification authority, closed the cross-org listing leak, and updated the security test to assert structural per-tenant isolation. (Corrected 2026-07-23: the original "wire it into the request path" already shipped 2026-07-22, commit `6828ca7` — see `docs/agent_card_design.md` §"Current State & Identity Hardening".) **Prior state:** every `NegotiationMessage` is already signed at creation with a per-agent AgentCard key, bound to `org_id`/`owner_user_id`, with an org-tenancy check at sign time (`session_manager._persist_message`). **The real remaining gap:** the API defaults `buyer_agent_id`/`seller_agent_id` to shared constants (`buyer-agent-001`/`seller-agent-001`), so agents in different orgs collide on one key file — whichever org signs first owns it, and every other org's messages then fail the tenancy check and silently do not sign (no ledger entry). **Fix:** provision one org-scoped `AgentIdentityRecord` per `(org_id, role)` — extend that table with `org_id`/`owner_user_id`/`public_key` via an Alembic migration — use its UUID as the signing identity, and make that DB row the authority `verify_agent_card` checks against instead of the card file's own `org_id`. Also close the cross-org listing in `routes/agent_cards.py` and `routes/identities.py` (both currently list all orgs' cards/identities unfiltered), and update `test_agent_card_cross_org_message_signing_blocked` (its "signing blocked" assertion becomes "structurally isolated per tenant"). Still the highest-ROI identity item — it's the prerequisite for a real cross-session reputation story (Phase 4).
...
- [ ] **Run `test_multitenant_load.py` for real, at a stated concurrency, write the number down in `docs/`.** ← The one genuinely-open Phase 1 item. Needs a running server; produces a documented concurrency/latency number.
