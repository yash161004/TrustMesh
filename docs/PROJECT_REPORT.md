...
*   **ManipulationDetector — Benchmark Results, Regression & Caveats:** 
    ...
    **The Prompt-Tuning Trade-off:** Note: A prior prompt version (self-consistency sampling + a 10-example contamination-checked few-shot prompt) showed a recall regression to 0.67 (2 consistent runs, identical false negatives both times: 'The Q4 Rush' and 'The Exploding Offer', both Urgency-tactic scenarios) versus the pre-expansion self-consistency baseline of 1.00/1.00 (5 runs). Working hypothesis: the 10-example prompt, rebalanced during a contamination fix, no longer includes an Urgency-category anchor example, which may be diluting the model's attention away from that tactic category. This was resolved on 2026-07-21: swapping the redundant 'Protective Advisor' example for a single-turn Urgency anchor ('The Gradual Squeeze', adversarial-6) restored recall, confirmed by three consecutive 1.00/1.00/1.00 holdout runs (see docs/EVAL_RESULTS.md, prompt version `post-few-shot-expansion-swap`). The CI gate (recall < 0.80) correctly caught the regression before it reached production.
    ...
    | **ManipulationDetector** | 1.00* | 0.67* | 0.80* | 0.2161* | 0.2381* |
...
*   **Tier 3: Signed Agent Identity (AgentCard):**
    ...
    **Known limitation (multi-tenant identity collision).** Signing uses per-agent Ed25519 keys (keyed on `agent_id`), and cards are bound to `org_id`/`owner_user_id` with an org-tenancy check enforced at sign time in `_persist_message` (wired 2026-07-22, commit `6828ca7`). A real ERC-8004 implementation would additionally give every agent its own on-chain-registered keypair under separate custody. The residual limitation in the current design is that the API defaults `agent_id` to shared constants (`buyer-agent-001`/`seller-agent-001`), so two orgs using the same default map to the same key file: the org-tenancy check then blocks all-but-the-first org from signing (a cross-tenant collision/lockout, not a forge), and key custody is by shared-filesystem convention rather than per-tenant separation. This is an explicit, acknowledged limitation, not an oversight. It is acceptable for the current local scope because (a) all agents are controlled by the same deployer, and (b) the primary threat model is external tampering with the card *after issuance*. Provisioning one org-scoped identity per `(org_id, role)` in a DB-backed registry is the corrected Phase 1 item #1 (see `docs/agent_card_design.md` §"Current State & Identity Hardening") and the prerequisite for a genuine multi-tenant or cross-org deployment.

## 5. KNOWN LIMITATIONS
*   **Confidence Calibration (ManipulationDetector) — Corrected:** Note: The recall regression identified in the 10-example few-shot prompt (0.67 recall, missing both Urgency-tactic scenarios — 'The Q4 Rush' and 'The Exploding Offer') has been resolved. Root cause: the contamination fix that swapped two few-shot examples for non-overlapping equivalents inadvertently removed the prompt's only Urgency-category anchor. Fix: swapped 'Protective Advisor' (a redundant Helpfulness-Disguise variant) for 'The Gradual Squeeze' (adversarial-6), using its final-turn message as a single-turn Urgency anchor. Note: adversarial-6 is tagged as multi-turn escalation (category 11) in the source data, not native Urgency (category 2) — it is used here because its final turn structurally matches the Urgency pattern, not because it is a native example of that category.\n\nConfirmed via 3 consecutive runs on the 8-scenario holdout: 1.00 precision / 1.00 recall / 1.00 F1 / 0.00 disagreement in all 3, with both previously-missed scenarios now caught and no regressions on any other scenario. (The original regression was observed across 3 pre-fix runs: two full runs at 0.67 recall, and one partial run cut short by API rate limits at 0.33 recall. Across all three, the only scenarios ever missed were the same two Urgency cases). As with all figures in this document, this reflects performance on the existing 8-scenario adversarial holdout, not a claim of generalization to novel scenarios. See Caveat B under Key Engineering Findings for the full explanation of the correction. The earlier report of poor calibration was itself an artifact of incorrect metric computation, not a real model deficiency.
...
*   **Open Problem in Manipulation Detection:** Note: The recall regression identified in the 10-example few-shot prompt (0.67 recall, missing both Urgency-tactic scenarios — 'The Q4 Rush' and 'The Exploding Offer') has been resolved. Root cause: the contamination fix that swapped two few-shot examples for non-overlapping equivalents inadvertently removed the prompt's only Urgency-category anchor. Fix: swapped 'Protective Advisor' (a redundant Helpfulness-Disguise variant) for 'The Gradual Squeeze' (adversarial-6), using its final-turn message as a single-turn Urgency anchor. Note: adversarial-6 is tagged as multi-turn escalation (category 11) in the source data, not native Urgency (category 2) — it is used here because its final turn structurally matches the Urgency pattern, not because it is a native example of that category.\n\nConfirmed via 3 consecutive runs on the 8-scenario holdout: 1.00 precision / 1.00 recall / 1.00 F1 / 0.00 disagreement in all 3, with both previously-missed scenarios now caught and no regressions on any other scenario. (The original regression was observed across 3 pre-fix runs: two full runs at 0.67 recall, and one partial run cut short by API rate limits at 0.33 recall. Across all three, the only scenarios ever missed were the same two Urgency cases). As with all figures in this document, this reflects performance on the existing 8-scenario adversarial holdout, not a claim of generalization to novel scenarios.
*   **Multi-Tenant Identity Collision & AgentCard Scope:** AgentCard signing uses per-agent Ed25519 keys with org-bound cards and an org-tenancy check at sign time (wired 2026-07-22). The residual limitation is that default `agent_id`s are shared constants, so distinct tenants collide on one key file and all-but-the-first org are locked out of signing. See the "Tier 3" section under Key Engineering Findings for the full analysis and the per-`(org, role)` DB-backed fix.
