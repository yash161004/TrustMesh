...
async def run_benchmark():
    ...
    scenarios_path = Path(__file__).parent.parent / "tests" / "benchmark_data" / "adversarial_round2_trust.json"
    ...
    for item in data:
        ...
        cat = "Manipulative" if expected_flagged else "Benign"
        ...
        if is_flagged and expected_flagged:
            ...
            false_results.append(f"FALSE POSITIVE | {current_scenario_id}: {item['name']} - Reason given: {result.get('reason', '')}")
    ...
    print("\n" + "=" * 60)
    print("MANIPULATION DETECTOR - ROUND 2 ADVERSARIAL BENCHMARK")
    print("=" * 60)
